...
export const formatDatetime = (date, symbol, apm) => {
...
export const formatTimeByHMS = (time, symbol) => {
    let result = '';
    if (time === undefined) {
        ...
        if (s.length < 2)
            s = '0' + s;
        result = [h, m, s].join(symbol !== null && symbol !== void 0 ? symbol : ':');
    }
    else {
        ...
        result = [hour, min, '00'].join(symbol !== null && symbol !== void 0 ? symbol : ':');
    }
    return result;
};
                ...
                apiAJAX({
                    ...
                    data.map((x) => {
                        t += `<li
                                ...
                                onclick="${gvc.event(() => {
                            if (set.multi) {
                                if (Array.isArray(set.def) && set.def.findIndex((z) => ObjCompare(z, x)) == -1) {
