...
init((gvc, glitter, gBundle) => {
    var _a;
    let mode = (_a = glitter.getCookieByName('EditMode')) !== null && _a !== void 0 ? _a : 'gui';
    gvc.addStyle(`
    .swal2-title {
                    ...
                    if (viewModel.loading) {
                        ...
                        >
                            <div class="swiper-wrapper">
                                ${gvc.bindView(() => {
                                let items = [];
                                function addItems(dd, array) {
                                    if (glitter.getCookieByName('lastSelect') === dd.id) {
                                        viewModel.selectItem = dd;
                                    }
                                    if (dd.type === 'container') {
                                        return {
                                            'text': dd.label, option: dd.data.setting.map((d4) => {
                                ...
                                viewModel.data.config.map((d3) => {
                                    items.push(addItems(d3, viewModel.data.config));
                                });
                                items = [
                                    {
                                        title: '首頁',
...
<i class="fa-sharp fa-solid fa-circle-plus " ></i>
                                                        ...
                                                        return `<li class=" list-group-item list-group-item-action border-0 py-2 px-4 ${d2.select ? `${(inner) ? `bg-warning` : `active`}` : ``} position-relative"
                                                                    onclick="${gvc.event(() => {
                                                            clearSelect();
                                                            d2.select = true;
                                                            parentCallback();
