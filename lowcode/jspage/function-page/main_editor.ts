...
export class Main_editor {
  ...
  padding: 0;
                                                                    ...
                                                                    Sortable.create(document.getElementById(id), {
                                                                        ...
                                                                        onChange: function (evt: any) {
                                                                            ...
                                                                            swapElements(
                                                                                og_array[startIndex].editor_bridge.element().parentNode,
                                                                                og_array[evt.newIndex].editor_bridge.element().parentNode
                                                                            );
                                        ...
                                        return html` <div class="p-2">${renderItems(list, pageConfig, true)}</div>`;
                                    })(),
                                ].join('');
                            })()}`,
                        ].join('');
                        ...
                        return html`
                            <li class="w-100 align-items-center  d-flex editor_item_title  start-0 bg-white z-index-9" style="z-index: 999;" onclick="${gvc.event(() => {})}">
                ...
                view: () => {
                    return html`
                        <div class="" style="">
                            ${(() => {
                                if (vm.type === 'list') {
                                    return [Main_editor.color_list(vm, gvc, id, globalValue), CustomStyle.globalContainerList(vm, gvc, id, globalValue)].join('');
    ...
    public static color_detail_custom(vm: { gvc: GVC; back: () => void; name: string; data: any; index: number; filter?: (key: string) => boolean }) {
        const gvc = vm.gvc;
            ...
            .map((dd) => {
                ...
                return EditorElem.colorSelect({
                    ...
                    callback: (text) => {
                        ...
                        if (`${vm.index}` === '0') {
                            (document.querySelector('#editerCenter iframe') as any).contentWindow.document.querySelector('body')!.style.background =
                                gvc.glitter.share.globalValue[`theme_color.0.background`];
                        }
                        vm.back();
                    },
                    gvc: gvc,
    ...
    public static color_detail(vm: { gvc: GVC; back: () => void; name: string; data: any; index: number; filter?: (key: string) => boolean }) {
        const gvc = vm.gvc;
        const globalValue = gvc.glitter.share.editorViewModel.appConfig;
        globalValue.color_theme = globalValue.color_theme ?? [];
        return gvc.bindView(() => {
  ...
  <div style="flex: 1 1 0; flex-direction: column; justify-content: center; align-items: flex-start; gap: 4px; display: inline-flex">
    <div style="align-self: stretch; color: #393939; font-size: 16px;  font-weight: 400; word-wrap: break-word">${vm.name}</div>
                                    ...
                                    `,
                        `<div style="padding: 18px 24px 24px;">${EditorConfig.color_setting_config
                            .filter((dd) => {
                                return !vm.filter || vm.filter(dd.key);
                            })
                            .map((dd) => {
    ...
    public static colorCard(cf: {
        ...
        cf = (cf || {}) as any;
        return `<div style="width:100%;padding: 11px 18px;background: ${
            cf.background || 'white'
        }; border-radius: 7px; overflow: hidden; border: 1px #DDDDDD solid; justify-content: center; align-items: center; display: flex">
    ...
    public static editorContent(option: { gvc: GVC; viewModel: any }) {
        ...
        return [
            html`
                <div
                    class="right_scroll"
                    style="overflow-y:auto;${Storage.select_function === 'user-editor' ? `height:calc(100vh - ${document.body.clientWidth < 768 ? 0 : 56}px)` : `height:calc(100vh - 150px);`}"
                    onscroll="${gvc.event(() => {
...
請直接點擊頁面元件，或於左側頁面區段來選擇元件進行編輯。</div>
                    ...
                    <div class="position-absolute w-100 bottom-0 d-flex align-items-center p-3 shadow justify-content-end border-top bg-white" style="height: 60px;">
                        ${BgWidget.cancel(
                            gvc.event(() => {
                                const dialog = new ShareDialog(gvc.glitter);
                        ...
                        ${viewModel.selectItem.deletable !== 'false'
                            ? ` <div class="mx-2"></div>` +
                              BgWidget.cancel(
                                  gvc.event(() => {
    ...
    public static center(viewModel: any, gvc: GVC) {
        ...
        >
            <div class="position-relative" style="width:100%;height: calc(100%);" id="editerCenter">
                <iframe class="w-100 h-100  bg-white" src="${gvc.glitter.root_path}${gvc.glitter.getUrlParameter('page')}?type=htmlEditor&appName=${gvc.glitter.getUrlParameter('appName')}"></iframe>
            </div>
        </div>`;
    }
                    ...
                    return gvc.bindView(() => {
                        ...
                        return {
                            ...
                            view: () => {
                                ...
                                let array = [];
                                if (!vm.selectItem) {
                                    array.push(html` <div class="d-flex bg-white w-100 border-bottom ${Storage.select_function === 'user-editor' ? `d-none` : ``}">
...
"
                                            ...
                                            (() => {
                                                ...
                                                return gvc.bindView(() => {
                                                    ...
                                                    return {
                                                        ...
                                                        view: () => {
                                                            return html`
                                                                <div class="alert alert-danger flex-fill m-0 p-2 " style="white-space: normal;word-break:break-all;">
                ...
                divCreate: option.divCreate || {
                    class: `w-100`,
                    style: `height:calc(100vh - ${document.body.clientWidth < 768 ? 0 : 56}px);overflow-y:auto;`,
                },
            };
        });
    }
