                                        ...
                                        [
                                            ...
                                            gvc.bindView(() => {
                                                ...
                                                return {
                                                    bind: id,
                                                    view: async () => {
                                                        let module_list = (await ApiPageConfig.getPageTemplate({
                                                            template_from: 'all',
                                                            page: '0',
                                                            limit: '3000',
                                                            type: 'module',
                                                        ...
                                                        if (dd.tag === '標頭元件') {
                                                            //參照元件
                                                            const widget = (await ApiPageConfig.getPage({
                                                            ...
                                                            //主元件
                                                            const refer_widget = (await ApiPageConfig.getPage({
                                                                appName: widget.config[0].data.refer_app,
                                                                type: 'template',
                                                                tag: widget.config[0].data.tag
                                                            })).response.result[0]

                                                            const find_showed_widget = (gvc.glitter.share.findWidget((dd: any) => {
                                                            ...
                                                            // console.log(`widget->標頭元件-find->${widget.config[0].id}->`,gvc.glitter.share.findWidget((dd:any)=>{
                                                            //     return dd.data && dd.data.tag==='c_header'
                                                            // }))
                                                            // gvc.glitter.share.editorViewModel.data
