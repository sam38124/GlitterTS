      ...
      return vm.dataList.map((dd: any) => {
        return [
          ...
          },
          {
            key: '信箱帳號',
            value:
              dd.email || dd.config.verifyEmail
          ...
          return BgWidget.container(
            [
              ...
              [
                      ...
                      return {
                        ...
                        view: () => {
                          ...
                          return BgWidget.inlineCheckBox({
                            ...
                            callback: text => {
                              vm.data.config.support_shop = text as any;
                              gvc.notifyDataChange(id);
                            },
                          });
                        },
                ...
                vm.data.id !== -1
                  ? BgWidget.mainCard(
                      html` <div class="tx_700">權限指派</div>
                      ...
                      gvc.event(() => {
                        dialog.warningMessage({
                          ...
                          callback: bool => {
                            if (bool) {
                              dialog.dataLoading({ visible: true });
                              ApiUser.deletePermission(
                                vm.data.email || vm.data.config.verifyEmail || vm.data.user
                              ).then(res => {
                                dialog.dataLoading({ visible: false });
                                if (res.result && res.response.result) {
                                  dialog.successMessage({ text: '刪除成功' });
                      ...
                      gvc.event(() => {
                        dialog.warningMessage({
                          ...
                          callback: bool => {
                            if (bool) {
                              dialog.dataLoading({ visible: true });
                              ApiUser.deletePermission(vm.data.email || (vm.data.config as any).verifyEmail).then(
                                res => {
                                  dialog.dataLoading({ visible: false });
                                  if (res.result && res.response.result) {
                                    dialog.successMessage({ text: '刪除成功' });
