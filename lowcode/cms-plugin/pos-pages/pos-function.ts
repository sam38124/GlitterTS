...
export class PosFunction {
  ...
  // 選擇多種付款方式
  public static selectPaymentMethod({
    gvc,
    orderData,
    callback,
  }: {
  ...
  public static selectVoucherAndRebate(obj: { gvc: GVC; notifyID: string; orderData: OrderDetail }) {
    ...
    const selectList = [
      ...
      },
      {
        title: '手動折扣',
        event: (callback: () => void) => {
          const hasManual = obj.orderData.voucherList.find((dd: any) => dd.id == 0);
          if (hasManual) {
              ...
              gvc.addMtScript(
                ...
                () => {
                  ApiShop.getVoucher({
                    ...
                    date_confirm: true,
                    user_email:
                      (orderDetail.user_info.email !== 'no-email' && orderDetail.user_info.email) ||
  ...
  // 手動折扣
  public static manualDiscount(obj: {
    gvc: GVC;
    orderDetail: any;
    reload: (voucher: any) => void;
                ...
                <div
                  ...
                  onclick="${gvc.event(() => {
                    ...
                    voucher.title =
                      voucher.title ||
                      `調整金額${
                        vm.tab === 'fixed' ? (vm.type === 'minus' ? '減少' : '增加') : '百分比'
                      }${vm.discount}${vm.tab === 'fixed' ? '元' : '%'}`;
                    orderDetail.voucherList = [...(orderDetail.voucherList ?? []), voucher];
                    gvc.closeDialog();
                    obj.reload(voucher);
                  })}"
                >
  ...
  // 手動折扣優惠券資料
  public static getNewVoucher = (method: 'fixed' | 'percent') => {
    return {
      id: 0,
      discount_total: 0,
      end_ISO_Date: '',
