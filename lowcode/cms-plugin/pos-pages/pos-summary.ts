...
<div><input class="form-control" style="width:100px;" placeholder="0" onclick="${gvc.event((e, event) => {
    ...
    public static async checkSummary(check_list: any, gvc: GVC, refresh: () => void) {
        ...
        ApiPos.getSummary(POSSetting.config.where_store).then(async (r) => {
            ...
            order.response.data.filter((dd:any)=>{
...
<input class="form-control flex-fill ms-2" style="" placeholder="請輸入備註" onchange="${gvc.event((e, event) => {
    ...
    public static showInfo(gvc: GVC, data: any, last: boolean) {
        ...
        const export_ = checkSummaryDir.check_list.find((dd: any) => {
            return dd.key === 'cash'
        }).export || 0
        BgWidget.settingDialog({
            gvc: gvc,
                ...
                view: () => {
                    ...
                    return BgWidget.container(html`
                        <div class="title-container ${PosSummary.in_pos() ? `mb-4`:''}">
                            ${BgWidget.title('小結紀錄')}
                            <div class="flex-fill"></div>
                            ${PosSummary.in_pos() ? BgWidget.darkButton(
                                    '新增小結單',
                                    obj.gvc.event(() => {
                        ...
                        ${(PosSummary.in_pos()) ? `` : BgWidget.tab(store_list.map((dd:any)=>{
                            return {
                                title:dd.name,
                                key:dd.id
                            }
