...
export class PosSummary {
  static in_pos() {
    return (window.parent as any).glitter.getUrlParameter('page') !== 'cms';
  }

  static addSummary(gvc: GVC, refresh: () => void) {
    const dialog = new ShareDialog(gvc.glitter);
    const oGvc = gvc;
    dialog.dataLoading({ visible: true });
    ApiPos.getSummary(POSSetting.config.where_store).then(async r => {
      const saasConfig: { config: any; api: any } = (window.parent as any).saasConfig;
      const data = (await saasConfig.api.getPrivateConfig(saasConfig.config.appName, 'glitter_finance')).response
        .result[0].value;
  ...
  static async checkSummary(check_list: any, gvc: GVC, refresh: () => void) {
    const dialog = new ShareDialog(gvc.glitter);
    dialog.dataLoading({ visible: true });
    ApiPos.getSummary(POSSetting.config.where_store).then(async r => {
      ...
      check_list.map((dd: any) => {
        const fi_ = last_.check_list.find((d1: any) => {
  ...
  static checkSummaryDir(gvc: GVC) {
    function refresh() {
      gvc.recreateView();
    }
  ...
  static showInfo(gvc: GVC, data: any, last: boolean) {
    function refresh() {
      gvc.recreateView();
    }
    ...
    return gvc.bindView(() => {
      ...
      async function initialStore() {
        if (!PosSummary.in_pos()) {
          ...
          gvc.glitter.share.member_auth_list = member_auth.response.data.filter((dd: any) => {
            return dd.invited && dd.status;
          });
          (window.parent as any).glitter.share.member_auth_list = gvc.glitter.share.member_auth_list;
          const store = store_d.response.value.list ?? [];
          store_list = store.filter((dd: any) => {
            return dd.is_shop;
          });
          ...
          POSSetting.config.where_store = store.find((dd: any) => {
            return dd.is_shop;
          }).id;
          vm.loading = true;
          gvc.notifyDataChange(init_id);
                                  ...
                                  return [
                                    {
                                      ...
                                      value: html`<span class="fs-7"
                                        >${obj.gvc.glitter.ut.dateFormat(
                                          new Date(dd.created_time),
                                          'yyyy-MM-dd hh:mm'
                                        )}</span
                                      >`,
                                    },
...
(window as any).glitter.setModule(import.meta.url, PosSummary);
