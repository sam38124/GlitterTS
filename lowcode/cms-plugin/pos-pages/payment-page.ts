...
export class PaymentPage {
    ...
    public static main(obj: { gvc: GVC; vm: ViewModel; ogOrderData: OrderDetail }) {
        ...
        // 確保 payment 陣列存在並檢查每個付款項
        const payments = obj.ogOrderData.pos_info.payment ?? [];
        payments.forEach((payment: any) => {
            payment.paied = payment.total > 0; // 根據 total 設定 paied
        });
                    ...
                    return new Promise(async (resolve, reject) => {
                        ...
                        const orderDetail = await (async () => {
                            ...
                            obj.ogOrderData.lineItems = obj.ogOrderData.lineItems.filter((dd) =>
                                orderDetail.lineItems.some((d1: any) => {
                                                                            ...
                                                                            <div style="font-size: 14px;height: 20px;" class="fw-500 ${data.pre_order ? `text-danger` : ''}">
                                                                                庫存:${(() => {
                                                                                    if (`${data.show_understocking}` === 'false') {
                                                                                        ...
                                                                                        return (
                                                                                            (data.stockList[POSSetting.config.where_store] && data.stockList[POSSetting.config.where_store].count) || 0
                                                                                        );
                                                                                    }
                                                                                })()}
                                                                            </div>
                                                                                                                                    ...
                                                                                                                                    >
                                                                                                                                        ${(() => {
                                                                                                                                            const leadData = JSON.parse(
                                                                                                                                                JSON.stringify(vm.user_data.member_level.re_new_member)
                                                                                                                                            );
                                                                                                                                            const condition_val =
                                                                                                                                                leadData.og.condition.value.toLocaleString();
                                                                                                                                            if (leadData.og.condition.type === 'total') {
                                                            ...
                                                            return {
                                                                ...
                                                                view: () => {
                                                                    return new Promise(async (resolve) => {
                                                                        ...
                                                                        resolve(
                                                                            form_array_view.join(html`<div class="my-4 border"></div>`) +
                                            ...
                                            view: () => {
                                                let view = [
                                                    html` <div class="d-flex" style="font-size: 18px;font-weight: 400;margin-bottom: 12px;">
                                                        <div>總金額</div>
                                                        <div class="ms-auto" style="font-size: 24px;font-weight: 700;">${parseInt(orderDetail.total as any, 10).toLocaleString()}</div>
                                                    </div>`,
                                                ...
                                                function paymentNext(pre_order: boolean) {
                                                    ...
                                                    if (
                                                        ...
                                                        if (obj.ogOrderData.pos_info.payment.find((dd: any) => dd.method === 'cash')) {
                                                        ...
                                                        } else if (obj.ogOrderData.pos_info.payment.find((dd: any) => dd.method === 'creditCard')) {
                                                        ...
                                                        } else if (obj.ogOrderData.pos_info.payment.find((dd: any) => dd.method === 'line')) {
                                                        ...
                                                        <div
                                                            ...
                                                            onclick="${gvc.event(() => {
                                                                if (total - parseInt(orderDetail.total as any, 10) < 0) {
                                                                    dialog.errorMessage({ text: `收款金額尚需要『 ${((total - parseInt(orderDetail.total as any, 10)) * -1).toLocaleString()} 』` });
                                                                    return;
                                                                } else if (
                                                                    obj.ogOrderData.pos_info.payment.length > 1 &&
                                                                    obj.ogOrderData.pos_info.payment.find((dd: any) => {
    ...
    public static scanVoucher(gvc: GVC, orderDetail: any, reload: () => void) {
        ...
        gvc.glitter.innerDialog(
            (gvc: GVC) => {
                async function next() {
                    await PaymentPage.enterCodeEvent(gvc, orderDetail, c_vm.value, reload);
                }
    ...
    public static async selectInvoice(gvc: GVC, orderDetail: any, vm: any, passData: any) {
        ...
        function handleSuccessfulCheckout(res: any) {
            ...
            // 顯示成功對話框
            gvc.glitter.innerDialog((gvc: GVC) => createSuccessDialog(gvc), 'orderFinish', { dismiss: () => {} });
        }

        function printInvoice(invoice: any, orderID: any, glitter: any) {
        ...
        function createSuccessDialog(gvc: GVC) {
            return html`
        ...
        // 不開立電子發票直接執行
        const invoiceTypeResponse = await ApiShop.getInvoiceType();
        const isNoInvoice = invoiceTypeResponse.response.method === 'nouse';

        const totalPayment = orderDetail.pos_info.payment.reduce((accumulator: any, payment: any) => accumulator + payment.total, 0);
