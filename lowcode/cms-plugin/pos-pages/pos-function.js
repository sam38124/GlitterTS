...
`;
function getVoucherText(gvc, voucherData) {
    ...
    const getDiscountDescription = (voucherData) => {
        if (voucherData.method === 'fixed') {
            return `折扣 ${voucherData.value} 元`;
        }
        return `符合條件商品折扣 ${voucherData.value}％`;
    };
    const getRuleDescription = (voucherData) => {
        const ruleMap = {
            min_price: '最少消費金額',
            min_count: '最少購買數量',
        };
    ...
    const getEndTimeDescription = (voucherData) => {
        if (!voucherData.endDate)
            return '無期限';
        return `${voucherData.endDate || '未設定日期'} ${voucherData.endTime || '尚未設定時間'}`;
    };
    const getProductForDescription = (voucherData, productForList) => {
    ...
    const translations = {
        ...
        'en-US': [
            getProductForDescription(voucherData, productForList),
            `${voucherData.rule === 'min_price' ? 'Minimum Spending Amount' : 'Minimum Purchase Quantity'} ${voucherData.rule === 'min_price' ? `${voucherData.ruleValue} USD` : `${voucherData.ruleValue} items`}`,
...
export class PosFunction {
    static switchUser(gvc, user) {
        const refresh = () => gvc.recreateView();
        gvc.glitter.innerDialog(gvc => {
            const c_vm = { text: '', id: gvc.glitter.getUUID() };
            const renderPinDots = () => Array.from({ length: 6 })
                .map((_, i) => html `<div
                style="width: 18px; height: 18px; background: ${c_vm.text.length > i
                ? '#FFB400'
                : '#B0B0B0'}; border-radius: 30px"
              ></div>`)
                .join('');
            const renderKeypad = () => {
                const keys = [
            ...
            const handleKeyPress = (key, isDelete) => {
                if (key === '取消')
                    return gvc.closeDialog();
                if (isDelete) {
            ...
            const verifyPin = () => {
                ...
                ApiUser.login({
                    ...
                    const auth = yield ApiUser.checkAdminAuth({
          ...
          <div style="display: flex; flex-direction: column; align-items: center; gap: 42px;">
            <div style="display: flex; flex-direction: column; align-items: center; gap: 18px;">
              <div style="display: flex; align-items: center; gap: 16px;">
                <div style="text-align: center;">
                  ...
                  <span style="color: #FFB400; font-size: 42px; font-family: Lilita One; letter-spacing: 2.52px;"
                    >X</span
                  >
                </div>
              ...
              <div
                style="text-align: center; color: #393939; font-size: 24px; font-weight: 700; letter-spacing: 2.4px;"
              >
                請輸入員工密碼
              </div>
            </div>
            ...
            <div
              style="background: white; border-radius: 10px; border: 1px solid #DDD; display: flex; flex-direction: column; align-items: center;"
            >
              ${renderKeypad()}
            </div>
          </div>
    ...
    static selectUserSwitch(gvc) {
        ...
        gvc.glitter.innerDialog(gvc => {
            ...
            const members = gvc.glitter.share.member_auth_list.filter((member) => member.user !== POSSetting.config.who);
            return gvc.bindView(() => ({
                bind: c_vm.id,
                view: () => {
                ...
                <div
                  ...
                  onclick="${gvc.event(() => PosFunction.switchUser(this_gvc, member.user))}"
                >
                  <img
                    src="https://assets.imgix.net/~text?bg=7ED379&amp;txtclr=ffffff&amp;w=100&amp;h=100&amp;txtsize=40&amp;txt=${member
                            .config.name}&amp;txtfont=Helvetica&amp;txtalign=middle,center"
              ...
              `;
                    ...
                    return html `
            <div
              class="w-100 position-absolute vw-100 vh-100"
              style="left: 0; top: 0; z-index: 0;"
              onclick="${gvc.event(() => gvc.closeDialog())}"
            ></div>
            <div
              class="w-100"
              style="flex-direction: column; display: flex; z-index: 1; position: relative;"
              onclick="${gvc.event((e, event) => event.stopPropagation())}"
            >
              ${memberElements}
            </div>
          `;
    ...
    static selectStoreSwitch(gvc, callback) {
        gvc.glitter.innerDialog(gvc => {
            ...
            const phoneCardStyle = isDesktop
                ? ''
                : 'border-radius: 10px; background: #fff; padding: 16px; box-shadow: 0px 0px 10px 0px rgba(0, 0, 0, 0.08);';
            gvc.addStyle(`
                .${c_vm.prefix}-title {
                    color: #393939;
                    text-align: center;
        ...
        <div>
          ...
          <div class="row mt-4">
            ${gvc.glitter.share.store_list
                .filter((store) => gvc.glitter.share.select_member.config.support_shop.includes(store.id))
                .map((store) => {
    ...
    static selectPaymentMethod({ gvc, orderData, callback, }) {
        ...
        gvc.glitter.innerDialog(gvc => {
            return gvc.bindView(() => {
                ...
                const renderPaymentMethod = (method, index) => {
                    return html ` <div
            class="d-flex align-items-center justify-content-center w-100 mt-lg-3 mt-1"
            style="height: 60px;"
          >
            ...
            <div class="d-flex" style="flex: 16;">
              ${paymentMethods.length > 1 ? renderDeleteIcon(method, index, gvc, dialog, paymentMethods, id) : ''}
                ...
                const renderDeleteIcon = (method, index, gvc, dialog, paymentMethods, id) => {
                    return html ` <i
          ...
          ></i>`;
                ...
                <div
                  ...
                  style="color: #393939; gap: 10px; font-size: 18px; font-weight: 700; text-transform: uppercase;"
                >
    ...
    static selectVoucherAndRebate(obj) {
        const gvc = obj.gvc;
        const glitter = gvc.glitter;
        ...
        const selectList = [
            ...
            },
            {
                title: '手動折扣',
                event: (callback) => {
                    const hasManual = obj.orderData.voucherList.find((dd) => dd.id == 0);
                    if (hasManual) {
                        dialog.infoMessage({ text: '請先移除目前使用的手動折扣紀錄' });
                        return;
                    }
                    PosFunction.manualDiscount({
                        gvc,
                        orderDetail: obj.orderData,
                        reload: () => {
                            gvc.notifyDataChange(obj.notifyID);
                            callback();
                        },
                    ...
                    view: () => {
                        try {
                            return [
                                ...
                                html `<div
                  style="color: #8D8D8D; text-align: center; font-size: 16px; font-weight: 400; margin: 8px 0 24px;"
                >
                  請注意，暫存訂單將保留 1 天，逾時則需重新建立。
                </div>`,
                  ...
                  <div class="d-flex" style="flex: 34px;"></div>
                                ...
                                TempOrder.getTempOrders()
                                    ...
                                    try {
                                        ...
                                        return html ` <div
                        class="d-flex align-items-center justify-content-center w-100 mt-lg-4 mt-1"
                        style="height: 40px;"
                      >
                        <div
                          class="d-flex"
    ...
    static userVouchers(gvc, orderDetail, reload) {
        const glitter = gvc.glitter;
        const vm = {
            dataList: [],
        };
                        ...
                        <div class="row">
                          ...
                          <div class="col-5 d-flex flex-column justify-content-center">
                            ${orderDetail.code_array.includes(content.code)
                                        ? PosWidget.buttonSnow('已套用', gvc.event(() => __awaiter(this, void 0, void 0, function* () { })))
                                        : PosWidget.buttonBlack('套用優惠', gvc.event(() => __awaiter(this, void 0, void 0, function* () {
                                            yield PaymentPage.enterCodeEvent(gvc, orderDetail, content.code, reload);
                                        })))}
                          </div>
                        </div>
                      `;
                        ...
                        if (loadings.view) {
                            gvc.addMtScript([{ src: `${gvc.glitter.root_path}/jslib/lottie-player.js` }], () => {
                                ApiShop.getVoucher({
                                    ...
                                    limit: 10000,
                                    date_confirm: true,
                                    user_email: (orderDetail.user_info.email !== 'no-email' && orderDetail.user_info.email) ||
                                        orderDetail.user_info.phone ||
                                        'guest',
    ...
    static userRebate(gvc, orderDetail, reload) {
        const glitter = gvc.glitter;
        const dialog = new ShareDialog(glitter);
        const vm = {
            order: {},
        ...
        glitter.innerDialog(gvc => {
            return gvc.bindView({
              ...
              >
                ...
                <div>
                  <span
                    style="color: #393939; text-align: center; font-size: 16px; font-style: normal; font-weight: 400; line-height: 140%; letter-spacing: 0.64px;"
                    >持有購物金</span
                  >
                  <span
                    style="color: #393939; font-size: 24px; font-style: normal; font-weight: 700; line-height: normal; letter-spacing: 0.96px; text-transform: uppercase;"
                    >${vm.sum}</span
                  >
                  <span
                    style="color: #393939; text-align: center; font-size: 16px; font-style: normal; font-weight: 400; line-height: 140%; letter-spacing: 0.64px;"
                                        ...
                                        return [
                                            BgWidget.editeInput({
                                                ...
                                                callback: value => {
                                                    ...
                                                    if (vm.sum === 0) {
                                                        dialog.errorMessage({
                                                            text: `您的 ${glitter.share.rebateConfig.title} 為 0 點，無法折抵`,
                                                        });
                                                    }
              ...
              ></i>`,
                ...
                onCreate: () => __awaiter(this, void 0, void 0, function* () {
                    if (loadings.view) {
                        vm.order = yield PaymentPage.checkout({
                        ...
                        new Promise((resolve, reject) => {
                            ApiShop.getRebate({ userID: orderDetail.user_info.id }).then((res) => __awaiter(this, void 0, void 0, function* () {
            ...
            return gvc.bindView({
                ...
                view: () => {
                    if (loadings.view) {
                        ...
                        const voucher = this.getNewVoucher(vm.tab);
                        return [
                            html `<div
                class="d-flex align-items-end"
                style="margin: 24px 0; color: #393939; text-align: center; font-size: 24px; font-style: normal; font-weight: 700; line-height: 140%; letter-spacing: 2.4px;"
              >
                手動調整價格
                      ...
                      <div class="row mb-2">
                        <div class="col-6">
                          <div class="tx_normal mb-1">選擇增減</div>
                          ${BgWidget.select({
                                            gvc,
                                            callback: value => {
                                                vm.type = value;
                        ...
                        <div class="col-6">
                          <div class="tx_normal mb-1">調整模式</div>
                          ${BgWidget.select({
                                            gvc,
                                            callback: () => { },
                                            default: vm.tab,
                                            options: [{ key: 'fixed', value: '固定金額' }],
                                        })}
                        </div>
                      </div>
                    `,
                      ...
                      <div class="d-flex gap-1 w-100">
                        <div style="width: 35%;">原始金額</div>
                        <div style="width: 35%;">調整（${vm.tab === 'fixed' ? '元' : '%'}）</div>
                        <div style="width: 30%;">調整後價格</div>
                      </div>
                      <div class="d-flex align-items-center gap-1 w-100">
                        <div style="width: 30%;">${limit.toLocaleString()}</div>
                        <div style="width: 5%;">${vm.tab === 'fixed' ? (vm.type === 'minus' ? '－' : '＋') : 'X'}</div>
                        ${BgWidget.editeInput({
                                            gvc,
                                            title: '',
                                            default: `${vm.discount}`,
                        ...
                        <div style="width: 30%;">
                          ${(() => {
                                            if (vm.tab === 'fixed') {
                                                const fixedTotal = limit - vm.discount * (vm.type === 'minus' ? 1 : -1);
                                                return fixedTotal.toLocaleString();
                                            }
                                            return Math.round(limit * (vm.discount / 100)).toLocaleString();
                                        })()}
                        </div>
                      </div>
                    `,
              ...
              >
                ...
                <div
                  ...
                  onclick="${gvc.event(() => {
                                ...
                                voucher.discount_total = n * (vm.type === 'minus' ? 1 : -1);
                                voucher.title =
                                    voucher.title ||
                                        `調整金額${vm.tab === 'fixed' ? (vm.type === 'minus' ? '減少' : '增加') : '百分比'}${vm.discount}${vm.tab === 'fixed' ? '元' : '%'}`;
                                orderDetail.voucherList = [...((_a = orderDetail.voucherList) !== null && _a !== void 0 ? _a : []), voucher];
                                gvc.closeDialog();
                                obj.reload(voucher);
                            })}"
                >
              ...
              ></i>`,
                ...
                onCreate: () => __awaiter(this, void 0, void 0, function* () {
                    if (loadings.view) {
                        vm.order = obj.refreshCheckout
                            ? yield PaymentPage.checkout({
                                line_items: orderDetail.lineItems,
                                user_info: orderDetail.user_info,
                                code_array: orderDetail.code_array || [],
...
PosFunction.getNewVoucher = (method) => {
    return {
        id: 0,
        discount_total: 0,
        end_ISO_Date: '',
