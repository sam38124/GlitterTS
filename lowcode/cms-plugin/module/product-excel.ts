                            ...
                            if (row[1]) {
                                ...
                                if(data[index+1] && (!data[index+1][1])){
                                    ...
                                    while (data[ind_] && (!data[ind_][1])){
                                        ...
                                        if(! productData.specs.find((dd:any)=>{
                                    ...
                                    function updateVariants() {
                                        ...
                                        combinations.map((d1: any) => {
                                            const spec = productData.specs.map((dd:any) => {
                                                return d1[dd.title]
                                            })
                                            //如果沒有此規格
                                            if (!productData.variants.find((d2:any) => {
                                        ...
                                        productData.variants = productData.variants.filter((variant:any) => {
                                            let pass = true;
                                            let index = 0;
                                            for (const b of variant.spec) {
                                                if (
                                                    !productData.specs[index] ||
                                                    !productData.specs[index].option.find((dd: any) => {
                                                        return dd.title === b;
                                                    })
                                                ) {
                                                    pass = false;
                                        ...
                                        if (productData.product_category === 'kitchen') {
                                            if(productData.variants.length>1){
                                                productData.variants.map((dd:any) => {
                                                    dd.compare_price=0
                                                    dd.sale_price = dd.spec.map((d1:any, index:number) => {
                                                        return parseInt(productData.specs[index].option.find((d2:any) => {
                                                            return d2.title === d1
                                                        }).price ?? "0",10)
                                                    }).reduce((acc:number, cur:number) => acc + cur, 0);
                                                    dd.weight=parseFloat(productData.weight ?? '0');
                                                    dd.v_height=parseFloat(productData.v_height ?? '0');
                                                    dd.v_width=parseFloat(productData.v_width ?? '0');
                                                    dd.v_length=parseFloat(productData.v_length ?? '0');
                                                    (dd.shipment_type as any)=productData.shipment_type!!
                                                })
                                            }else{
                                                productData.variants.map((dd:any) => {
                                                    dd.weight=parseFloat(productData.weight ?? '0');
            ...
            response.response.data.forEach((productData: any) => {
                ...
                if(productData.content.specs.length){
                    ...
                    const baseRowData = (specs:any,option:any,index: number): RowInitKitchen => ({
                        ...
                        stockPolicy:
                            `${productData.content.variants[index]?.stock ?? ''}` ? '追蹤' : '不追蹤',
                        stock: expo.checkNumber(productData.content.variants[index]?.stock)
                    });
