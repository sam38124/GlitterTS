...
export class StockHistory {
  ...
  static checkList(gvc: GVC, vm: VM) {
    ...
    function checkSpecTable(page: number, limit: number) {
      ...
      return {
        array: specs.map((dd: ContentProduct, index: number) => {
                    ...
                    new Promise<[]>(resolve => {
                      ...
                      this.setVariantList(ids, vm.data, response => {
                        ...
                        const filterCriteria: { [key: string]: (item: any) => boolean } = {
                          ...
                          pending: item => {
                            return (
                              searchIncludes(item.title) &&
                              item.recent_count !== undefined &&
                              item.transfer_count > item.recent_count
                            );
                          },
                          notChecked: item => {
                        ...
                        vmi.tableData = criteria
                          ? checkSpec.array.filter((item, index) => criteria(checkSpec.data[index]))
                          : [];
                                    ...
                                    BgWidget.tableV3({
                                      ...
                                      getData: vd => {
                                        ...
                                        new Promise<[]>(resolve => {
                                          ...
                                          this.setVariantList(dvm.variantIds, vm.data, response => {
                                            response.map(item1 => {
                                              const stockData = responseArray.find(
                                                (item2: any) => item1.variant_id === item2.id
                                              );
                                ...
                                case 'checking':
                                  return [
                                    EditorElem.radio({
                                      ...
                                      array: productSelect,
                                      callback: text => {
                                        vm.data.content.check_according = text as 'all' | 'collection' | 'product';
                                                            ...
                                                            BgProduct.collectionsDialog({
                                                              ...
                                                              callback: async value => {
                                                                ...
                                                                ApiShop.getCollectionProductVariants({
                                                                  tagString:
                                                                    subVM.def.length > 0 ? subVM.def.join(',') : '',
                                                                }).then(r1 => {
                                                                  if (
                                                                    r1.result &&
                                                                    r1.response &&
                                                                    r1.response.length > 0
                                                                  ) {
                                                                    dvm.variantIds = r1.response.map(
                                                                      (item: any) => item.id
                                                                    );
                                                                    const limit = 99999;
                                                                    ApiStock.getStoreProductList({
                                          ...
                                          case 'all':
                                            ApiStock.getStoreProductList({
                                              ...
                                              if (r.result && r.response.data) {
                                                ...
                                                dvm.variantIds = responseArray.map((item: any) => `${item.id}`);
                                                this.setVariantList(dvm.variantIds, vm.data, response => {
                                                  response.map(item1 => {
                                                    const stockData = responseArray.find(
                                                      (item2: any) => item1.variant_id === item2.id
  ...
  static integerColorComponent(n: number, leftNumber: number | undefined, rightNumber: number) {
    if (n === 0) {
      ...
      switch (json.type) {
        ...
        case 'logs':
          return [
            ...
            {
              key: '此次補貨數量',
              value: dd.replenishment_count
                ? html`<span class="fs-7 tc_success">+${dd.replenishment_count ?? 0}</span>`
                : html`<span class="fs-7">－</span>`,
            },
            ...endArr,
          ];
      }
  ...
  static async verifyCurrentStock(data: StockHistoryData): Promise<ContentProduct[]> {
        ...
        buttonList.push(
          BgWidget.save(
            gvc.event(async () => {
              ...
              const diffVariants = await this.verifyCurrentStock(vm.data);
              ...
              const diffVM = {
                dataList: [] as ContentProduct[],
              };

              function changeStockTable() {
                return diffVM.dataList.map((dd: ContentProduct) => {
                  const n = (dd.stock ?? 0) - dd.transfer_count;
                  return [
                    {
                      key: '商品',
                      value: `<span class="fs-7">${dd.title || '－'}</span>`,
                          ...
                          callback: bool => {
                            if (bool) {
                              ...
                              vm.data.content.product_list.map(item => {
                                ...
                                const n = (variant.stock ?? 0) - item.transfer_count;
        ...
        BgWidget.cancel(
          gvc.event(async () => {
            if (isEqual) {
              ...
              if (vm.data.status === 5) {
                if (vm.data.type === 'checking') {
                  const diffs = await this.verifyCurrentStock(vm.data);
                  if (diffs.length > 0) {
                    updateData(5);
                  } else {
                    updateData(0);
