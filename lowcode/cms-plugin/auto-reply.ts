                    ...
                    vm.dataList = await Promise.all(
                      [
                        ...
                        return new Promise(async resolve => {
                          const data = await AutoReply.getDefCompare(b);
                    ...
                    ${BgWidget.questionButton(
                      gvc.event(() => {
                        BgWidget.dialog({
                          ...
                          title: '模板字串使用提示',
                          innerHTML: () => {
                            return BgWidget.alertInfo(
                              '可使用模板字串，信件將在寄送時自動填入相關數值',
                              BgWidget.richTextQuickList.map(item => `${item.title}: ${item.value}`),
                              {
                                class: 'mb-3',
                                style: '',
                              }
                              ...
                              view: () => {
                                try {
                                  return BgWidget.richTextEditor({
                                    ...
                                    title: '內容編輯',
                                    quick_insert:
                                      ['get-customer-message', 'proof-purchase'].includes(tag) ||
