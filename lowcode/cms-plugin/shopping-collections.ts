...
export class ShoppingCollections {
                                                                ...
                                                                if (d.result) {
                                                                    ...
                                                                    ApiShop.getCollection().then((data) => {
                                                                        if (data.result && data.response.value.length > 0) {
                                                                            vm.allParents = ['(無)'].concat(data.response.value.map((item: {
                                                                                title: string
                                                                            }) => item.title));
    ...
    public static editorDetail(obj: { vm: ViewModel; gvc: GVC; type?: 'add' | 'replace' }) {
        ...
        const dialog = new ShareDialog(gvc.glitter);
        let select_lan = (window.parent as any).store_info.language_setting.def
                                ...
                                <div class="update-bar-container">
                                    ...
                                    ${BgWidget.save(
                                            gvc.event(() => {
                                                ...
                                                const no_fill_language = (window.parent as any).store_info.language_setting.support.find((dd: any) => {
