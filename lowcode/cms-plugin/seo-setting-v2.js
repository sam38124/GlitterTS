    ...
    static main(gvc) {
        ...
        function loadData() {
            ...
            getSEOData().then((r) => __awaiter(this, void 0, void 0, function* () {
                ...
                const ro_ = (yield ApiUser.getPublicConfig('robots_text', 'manager')).response.value;
                vm.robots = ro_.text || '';
                gvc.notifyDataChange(vm.id);
            }));
        }
                        ...
                        view: () => {
                            ...
                            return BgWidget.mainCard(html `
                                    <div class="d-flex flex-column" style="gap:18px;">
                                        ${BgWidget.editeInput({
                                gvc: gvc,
                                title: 'SEO標題' + BgWidget.languageInsignia(vm.language, 'margin-left:5px;'),
...
${BgWidget.languageInsignia(vm.language, 'margin-left:5px;')}
                                ...
                                imageLibrary.selectImageLibrary(gvc, (urlArray) => {
                                    if (urlArray.length > 0) {
                                        ...
                                        ${BgWidget.textArea({
                                gvc: gvc,
                                title: `<div class="d-flex align-items-center"><div class="tx_normal fw-bold" style="">自訂代碼區塊</div>${BgWidget.languageInsignia(vm.language, 'margin-left:5px;')}</div>`,
...
${BgWidget.grayNote('未輸入則參照系統預設')}
                                ...
                                placeHolder: `User-agent: * \nSitemap: https://${window.parent.glitter.share.editorViewModel.domain}/sitemap.xml`,
                            })}
                                    </div>
...
window.glitter.setModule(import.meta.url, SeoSettingV2);
