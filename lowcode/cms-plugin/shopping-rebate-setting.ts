...
export class ShoppingRebateSetting {
    public static main(gvc: GVC) {
        ...
        const vm = {
            ...
            dataObjectList: [] as { obj: any; key: string }[],
        };

        function formatObject(obj: any, parentObj = obj) {
        ...
        return gvc.bindView({
                                                                            ...
                                                                            return html`<div style="display: flex; flex-direction: column; gap: 18px;">
                                                                                ...
                                                                                <div>
                                                                                    ...
                                                                                    ${BgWidget.multiCheckboxContainer(
                                                                                        gvc,
                                                                                        [{ key: 'true', name: '無使用期限' }],
                                                                                        vm.data.register.unlimited ? ['true'] : [],
                                                                                        (list: any) => {
                                                                                                            ...
                                                                                                            if (levelVM.loading) {
                                                                                                                ApiUser.getPublicConfig('member_level_config', 'manager').then((dd: any) => {
                                                                                                                    if (dd.result && dd.response.value) {
                                                                                                                        levelVM.options = dd.response.value.levels.map(
                                                                                                        ...
                                                                                                        BgWidget.editeInput(
                                                                                                            (() => {
                                                                                                                function formatPercent(text: string | number) {
                                                                                                                    ...
                                                                                                                    if (isNaN(textInt)) return 0;
                                                                                                                    if (textInt < 0) return 0;
                                                                                                                    if (textInt > 100) return 100;
                                                                                                                    return textInt;
                                                                                                                }
            ...
            onCreate: () => {
                if (vm.loading) {
                    ApiUser.getPublicConfig('rebate_setting', 'manager').then((data: any) => {
...
(window as any).glitter.setModule(import.meta.url, ShoppingRebateSetting);
