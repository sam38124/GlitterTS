                ...
                [
                    BgWidget.mainCard(
                        [
                            ...
                            BgWidget.grayNote(
                                [`*透過AI Points來使用各種 AI 輔助功能`, `*1塊台幣可以換取10點AI Points`, `*最少儲值金額500元`, `*AI Points購買後無法退費，請購買前瞭解會員權益需知`]
                                    .map((dd) => {
                    ...
                    BgWidget.save(
                        gvc.event(async () => {
                            ...
                            if (gvc.glitter.deviceType === gvc.glitter.deviceTypeEnum.Ios) {
                                ...
                                AiPointsApi.store(vm).then(async (res) => {
                                    dialog.dataLoading({ visible: false });
...
(window as any).glitter.setModule(import.meta.url, AiPoints);
