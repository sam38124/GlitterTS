...
export class POSSetting {
    static main(gvc) {
        ...
        window.appName = 'shop-template-clothing-v3';
                    ...
                    return html `
                        <div class="d-flex nav-top"
                            ...
                            <div class="POS-logo h-100 d-flex align-items-center justify-content-center mx-2 w-100"
                                ...
                                <div class="searchBar ms-2 ${((document.body.offsetWidth < 800)) ? `flex-fill` : ``}"
                                     style="position: relative;max-width:calc(100% - 60px);">
                                    <input class="border-0" placeholder="搜尋商品名稱或貨號"
                                           style="display: flex;${(document.body.offsetWidth < 800) ? `width:100%;` : `width: 357px;max-width:calc(100%);`}height: 56px;padding: 17px 24px;justify-content: center;gap: 10px;border-radius: 10px;font-size: 18px;background: #F7F7F7;"
 ...
 font-size: 20px;"></i><span style="position: absolute;top:-32px;right: -12px;"><span style="display: inline-block;
    ...
    right: 0px;">${orderDetail.lineItems.length}</span></span></span>`;
                            ...
                            return page.map((data) => {
                                return html `
                                        <div class="d-flex flex-column justify-content-center align-items-center"
                                             style="padding: 16px 28px;gap: 6px;font-size: 20px;font-weight: 500;letter-spacing: 0.8px;${(vm.type == data.type) ? `color:#393939;${(document.body.offsetWidth < 800) ? `border-top: 3px solid #393939;` : `border-right: 3px solid #393939;`}` : 'color:#949494;'}"
                                             onclick="${gvc.event(() => {
                                    vm.type = data.type;
                                })}">
                        ...
                        view: () => __awaiter(this, void 0, void 0, function* () {
                            try {
                                if (vm.type == "payment") {
                                    const checkOutData = yield ApiShop.getCheckout({
