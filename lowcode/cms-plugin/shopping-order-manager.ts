...
export class ShoppingOrderManager {
                                        ...
                                        ApiShop.putOrder({
                                            ...
                                            if (response.result) {
                                                if (orderData.orderData.method && origData.status == 0 && orderData.status == 1) {
                                                    dialog.successMessage({ text: html`<div class="text-center">訂單付款完成！<br />若需要可透過下方按鈕手動建立發票</div>` });
                                                } else {
    ...
    static printStoreOrderInfo(obj: {
        ...
        ApiDelivery.getOrderInfo({
            ...
            dialog.dataLoading({ visible: false });
            if (res.result && res.response.id) {
                const url = ApiDelivery.getFormURL(res.response.id);
                glitter.openNewTab(url);
            } else {
                dialog.errorMessage({ text: '資料回傳錯誤' });
            }
        });
        return;
    }
