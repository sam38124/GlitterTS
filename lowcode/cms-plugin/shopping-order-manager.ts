...
export class ShoppingOrderManager {
    ...
    public static replaceOrder(gvc: GVC, vm: any, id: string) {
        const glitter = gvc.glitter;
        vm.data.orderData.progress=vm.data.orderData.progress ?? 'wait'
        vm.data.orderData.orderStatus=vm.data.orderData.orderStatus ?? '0'
...
`;
                        ...
                        html`
                            ...
                            return `<div style="border-radius: 7px;background: #EAEAEA;padding: 4px 6px;color:#393939;font-weight: 700;">一般會員</div>`
                                                                    ...
                                                                    <div class=""
                                                                        ...
                                                                        ${(orderData.orderData.customer_info && orderData.orderData.customer_info.phone) ||  (userData?.userData?.phone ?? orderData.orderData.user_info.phone) }
                                                                    </div>
                                                                    <div class=""
                                                                         style="color: #393939;font-weight: 400;">
                                                                        ${(orderData.orderData.customer_info && orderData.orderData.customer_info.email) ||  (userData?.userData?.email ?? orderData.orderData.user_info.email)}
                                                                    </div>
                                                                </div>`,
                        html`
                                                                <div class="bgf6"
                        ...
                        if (orderData.orderData.shipment_form_format && orderData.orderData.shipment_form_format.length > 0) {
                            ...
                            return ``
                                            ...
                                            ${(orderData.orderData.user_info.code_note) ? ` <div class="mt-2"></div>` + BgWidget.card(
                        gvc.bindView(() => {
                            const id = glitter.getUUID();
                            const vm: {
                                mode: 'edit' | 'read';
                            ...
                            return {
                                ...
                                view: () => {
                                    return html`
                                                                        ...
                                                                        <div class="fs-6 w-100 mt-2  lh-lg fw-normal"
                                                                             style="word-break: break-all;white-space:normal;color:#393939;">
                                                                            ${(orderData.orderData.user_info.code_note ?? '尚未填寫').replace(/\n/g, `<br>`)}
                                                                        </div>
                                                                    `;
                                },
                                divCreate: {
                        ...
                        function writeEdit(origData: any, orderData: any) {
                            ...
                            if ((orderData.orderData.orderStatus ?? '0') != (origData.orderData.orderStatus ?? '0')) {
                                let text: any = {
                                    '1': '訂單完成',
                                    '0': '訂單改為處理中',
                                    '-1': '訂單已取消',
                                                            ...
                                                            if (product.select) {
                                                                ...
                                                                return html`
                                                                    ...
                                                                    if (product.content.variants.length > 1) {
                                                                        product.selectIndex = (
                                                                            document.querySelector('.varitantSelect') as HTMLInputElement
                                                                        ).value;
                                                                    }
                                                                    gvc.notifyDataChange(`product${productIndex}`);
                                                                })}"
                                ...
                                dismiss: () => {
                                    ...
                                    if (newOrder.productCheck.length > 0) {
                                        const updateProductCheck = (tempData: any) => {
                                            const productData = newOrder.productCheck.find((p: any) => p.id === tempData.id && p.selectIndex === tempData.selectIndex);
