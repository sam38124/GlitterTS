...
`;
  ...
  static voucherEditor(obj: { vm: any; gvc: GVC; type?: 'add' | 'replace'; defData?: any }) {
    const gvc = obj.gvc;
    const glitter = gvc.glitter;
    const vm = obj.vm;
    const voucherData: {
                                                          ...
                                                          EditorElem.searchInputDynamic({
                                                            ...
                                                            callback: text => {
                                                              ApiShop.getProduct({
                                                                ...
                                                                voucherData.forKey[index] = data.response.data.find(
                                                                  (dd: any) => {
...
<span class="ms-2">個</span>
                        ...
                        return data.response.data.map((dd: any) => {
                          return [
                            ...
                            },
                            {
                              key: '商品',
                              value:
                                `<img class="rounded border me-2 ${dd.content.preview_image[0] ? `` : `d-none`}" alt="" src="${
  ...
  static collectionManager(obj: { gvc: GVC }) {
    let array: any = [];
  ...
  static showListManager(obj: { gvc: GVC }) {
    let array: any = [];
    const glitter = obj.gvc.glitter;
  ...
  static editProduct(obj: { vm: any; gvc: GVC; type?: 'add' | 'replace'; defData?: any }) {
    let postMD: {
      id?: string;
      title: string;
      content: string;
...
"
            ...
            <div style="width:300px;max-width:100%;">
              ${BgWidget.card(
                `  ${postMD.id ? html` ${EditorElem.h3('商品ID')} ${postMD.id} ` : ``}` +
                  EditorElem.select({
                    gvc: obj.gvc,
                    title: '商品狀態',
                    def: postMD.status,
  ...
  static invoice_setting(gvc: GVC) {
    const saasConfig: {
      config: any;
      api: any;
    } = (window as any).saasConfig;
  ...
  static logistics_setting(gvc: GVC) {
    const saasConfig: {
      config: any;
      api: any;
    } = (window as any).saasConfig;
        ...
        return {
          ...
          view: () => {
            return new Promise(async (resolve, reject) => {
              ...
              resolve(
                html` <div style="width:900px;max-width:100%;">
                  ${BgWidget.card(
                    [
