          ...
          return {
            ...
            view: () => {
              ...
              if (obj.readonly) {
                ...
                const emailHTML = vm.data.userList.map(
                  (data: any) => html` <div class="c_filter_tag">${data.email}</div>`
                );
                      ...
                      ApiUser.getUserListOrders({
                        ...
                        if (dd.response.data) {
                          userVM.dataList = dd.response.data.map(
                            (item: { userID: number; account: string; userData: { name: string; email: string } }) => {
                              return {
                                key: item.userID,
