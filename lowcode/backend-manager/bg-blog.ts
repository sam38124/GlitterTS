...
export class BgBlog {
  public static setCollection(cf: {
    goBack?: () => void;
    gvc: GVC;
    key: string;
    select_module?: {
                  ...
                  >
                    ...
                    >
                      <div
                        class="${allSelect({
                          items: vm.link,
                          selected: !vm.link.find(dd => !(dd as any).selected),
                        })
                        ...
                        onclick="${cf.gvc.event((e, event) => {
                          ...
                          if (vm.link.find(dd => !(dd as any).selected)) {
                            selectAll({ items: vm.link } as any);
                          } else {
                            clearAll({ items: vm.link } as any);
                          }
                          gvc.notifyDataChange(vm.id);
                        })}"
                      ></div>
                                                ...
                                                onclick="${gvc.event(() => {
                                                  if (dd.tag === 'empty') {
                                                    if (['hidden', 'shopping'].includes(page_tab)) {
                                                      const a: any = [
                                                        Template_refer.bigTitle(
                                                          page_tab === 'hidden' ? `隱形賣場` : `一頁商店`
                                                        ),
                                                    ...
                                                    if (dd._config) {
                                                      ...
                                                      BaseApi.create({
                                                        ...
                                                        res.response.result[0].config.splice(
                                                          res.response.result[0].config.length - 1,
                                                          1
                                                        );
...
function initialData(content: any) {
  function empty() {
    return {
      name: '',
      seo: {
                                                            ...
                                                            insertImageEvent: editor => {
                                                              ...
                                                              imageLibrary.selectImageLibrary(
                                                                ...
                                                                </div>`,
                                                                {
                                                                  ...
                                                                  cancelEvent: () => {
                                                                    editor.html.set(
                                                                      editor.html.get(0).replace(mark, '')
                                                                    );
                                                                    editor.undo.saveStep();
                                                                  },
                                                                }
                                            ...
                                            if (subVM.loading) {
                                              if (vm.data.content.relative_data.length === 0) {
                                                ...
                                                new Promise<OptionsItem[]>(async resolve => {
                                                  ...
                                                  vm.data.content.relative_data = vm.data.content.relative_data.filter(
                                                    (dd: any) => {
...
function saveData(gvc: GVC, cf: any, vm: any, cVm: any, silence: boolean, pass_check: boolean = false) {
  return new Promise(async resolve => {
    if (!vm.data.content.tag) {
      ...
      (window.parent as any).store_info.language_setting.support.find((d1: any) => {
