...
export class BgGlobalEvent {
    public static selectTag?: string = undefined
    public static saveEvent = () => {
    }
    public static refresh=()=>{}

    public static eventEditorView(gvc: GVC) {
        return html`
            ...
            <div class="p-2">${BgGlobalEvent.editor({
                gvc: gvc,
                tag: BgGlobalEvent.selectTag as any,
    ...
    public static mainPage(gvc: GVC) {
        ...
        return `<div class="d-flex w-100">
                <div style="width:300px;" class="">
                    ${BgGlobalEvent.leftBar(gvc, (tag?: string) => {
...
${gvc.bindView(() => {
    ...
    public static leftBar(gvc: GVC, callback: (tag?: string) => void) {
                ...
                view: () => {
                    return new Promise(async (resolve, reject) => {
                        if (res.result && res.response.result) {
                            let mapData: any = [];
                            res.response.result.sort(function (a: any, b: any) {
                                // 按姓名的字母顺序排序
    ...
    public static editor(obj: {
        ...
        return obj.gvc.bindView(() => {
            let vm: any = (obj.type === 'put') ? undefined : {
                data: {
                    tag: '',
                    name: '',
                    json: {
                    ...
                    if (!vm) {
                        ...
                        return [
                            ...
                            EditorElem.editeInput({
                                gvc: obj.gvc,
                                title: '事件分類',
                                default: vm.data.json.group ?? "",
                                placeHolder: '透過添加/往後延續子分類',
                                callback: (text) => {
                                    vm.data.json.group = text
                                }
                            }),
                            TriggerEvent.editer(obj.gvc, vm.data.json as any, vm.data.json as any, {
                                hover: false,
                                option: [],
                                title: "設定觸發事件"
                            }),
