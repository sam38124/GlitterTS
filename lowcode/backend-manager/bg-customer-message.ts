                                        ...
                                        view: () => {
                                            return new Promise((resolve, reject) => {
                                                ...
                                                gvc.glitter.getModule(url.href, (BgCustomerMessage) => {
                                                    resolve(
                                                        BgCustomerMessage.showCustomerMessage({
                                                            gvc: gvc,
                                                            userID: userID,
                                                            open: true,
                                                        })
...
`);
                    ...
                    return html`
                        <div class=" card rounded-3">
                            <div class="p-3 rounded-top" style="width: 100%;white-space: normal;background: ${BgCustomerMessage.config.color};">
                                <div class="text-white fw-bold" style=" font-size: 20px;">${BgCustomerMessage.config.title ?? ''}</div>
                                <p class=" text-white mt-2 mb-4" style=" font-size: 16px;line-height: 22px;font-weight: 300;">${(BgCustomerMessage.config.content ?? '').replace(/\n/g, `<br>`)}</p>
                                <button
    ...
    public static showCustomerMessage(cf: { gvc: GVC; userID: string; toUser?: string; viewType?: string; open?: boolean }) {
        const gvc = cf.gvc;
        const html = String.raw;
        cf.userID = `${cf.userID}`;
                ...
                onCreate: async () => {
                    ...
                    const roBotView = `<div class="position-fixed  rounded-3 shadow-lg " style="${gvc.glitter.ut.frSize(
                        ...
                        css`
                            ...
                            background: white;
                        `
                    )}">${BgCustomerMessage.robotMessage(gvc, async (text) => {
                        vm.viewType = 'message';
                        if (text) {
