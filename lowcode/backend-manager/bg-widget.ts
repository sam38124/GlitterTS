...
export class BgWidget {
    ...
    static plus_btn(title: string, gvc: GVC, option: {
        title: string,
        icon: string,
        callback: () => void
    }[]) {
        ...
        `
        // `<div class="w-100 " style=" justify-content: center; align-items: center; gap: 4px; display: flex;color: #3366BB;cursor: pointer;" onclick="${
        //     gvc.event(()=>{
        //         toggle()
        //     })
        // }">
...
${obj.default ?? ''}</textarea
    ...
    static alertInfo(title: string, messageList?: string[], css: { class: string; style: string } = {
        class: '',
        style: ''
    }) {
        let h = '';
