...
export class BgWidget {
    ...
    static darkButton(
        ...
        return html`
            <button class="btn btn-black ${size.btn} ${obj?.class ?? ''}" type="button" style="${obj?.style ?? ''}"
                    onclick="${event}">
    ...
    :
        string, event
    :
        string, obj ? : {icon? : string; textStyle? : string; class? : string; style? : string}
    )
        {
            return html`
    ...
    :
        string, event
    :
        string, obj ? : {icon? : string; textStyle? : string}
    )
        {
            return html`
                <button class="btn btn-red" type="button" onclick="${event}">
...
${obj.default ?? ''}</textarea
    ...
    :
        ...
        // 容器
    static
        getContainerWidth = (obj?: { rate?: { web?: number; pad?: number; phone?: number } }) => {
    ...
    :
        string, width ? : number, style ? : string
    )
        {
            return html`
                <div
    ...
    :
        string, classStyle
    :
        string = 'p-3 bg-white rounded-3 shadow border w-100'
    )
        {
            return html`
    ...
    static
        mainCard(htmlString
    :
        string, classString ? : string, styleString ? : string
    )
        {
            return html`
    ...
    :
        ...
        {
            return html`
                <div class="w-100 position-relative" style="height: 40px; margin: ${margin ?? 0};">
                    <i class="fa-regular fa-magnifying-glass"
...
<
    i
    class = "fa-regular fa-arrow-up-arrow-down" > </i>
        < /div>
    ${obj.gvc.bindView({
                bind: vm.id,
                view: () => {
...
)
{
    ...
    return obj.gvc.bindView({
        ...
        view: () => {
            if (imageVM.loading) {
                return html`
                    <div class="${imageVM.class} ${obj.class ?? ''}" style="${obj.style ?? ''}">
                        ${this.spinner({
                ...
                return html`<img class="${imageVM.class} ${obj.class ?? ''}" style="${obj.style ?? ''}"
                                 src="${imageVM.url}"/>`;
            }
        },
        divCreate: {},
