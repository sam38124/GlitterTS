...
export class BgWidget {
    ...
    static tab(
        ...
        return html`
            <div style="justify-content: flex-start; align-items: flex-start; gap: 22px; display: inline-flex;cursor: pointer;margin-top: 24px;margin-bottom: 24px; ${style ?? ''}">
                ${data
                        .map((dd) => {
                            if (select === dd.key) {
                                return html`
...
${obj.default ?? ''}</textarea
    ...
    static linkList(obj: {
        ...
        // 確認網址，回傳icon與父子層陣列
        const findMenuItemPathByLink = (items: MenuItem[], link: string): {
            icon: string;
            nameMap: string[]
        } | undefined => {
