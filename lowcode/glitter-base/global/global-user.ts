...
export class GlobalUser {
  ...
  public static set token(value) {
    ...
    if(!['cms_system'].includes((window as any).appName)){
      if(value){
  ...
  //註冊裝置推播
  public static registerFCM(value:string) {
      if (value) {
          try {
              (window as any).glitter.runJsInterFace(
                'getFireBaseToken',
                {},
                (response: any) => {
                    if (response.token) {
                        ApiUser.registerFCM(
                          GlobalUser.parseJWT(value).payload.userID,
                          response.token
                        );
                    }
                },
