...
if (['blogs', 'pages', 'shop', 'hidden'].find(dd => {
    return `${glitter.getUrlParameter('page')}`.startsWith(dd);
}) &&
    `${glitter.getUrlParameter('page')}`.split('/')[1]) {
    ...
    let lan_d = (await ApiUser.getPublicConfig(`terms-related-${page}-${Language.getLanguage()}`, 'manager'))
        .response.value.text;
    if (!lan_d) {
        lan_d = (await ApiUser.getPublicConfig(`terms-related-${page}-${window.store_info.language_setting.def}`, 'manager')).response.value.text;
    }
