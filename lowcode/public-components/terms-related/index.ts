    ...
    return gvc.bindView(() => {
      return {
        ...
        view: async () => {
          //分類頁面
          if (
            `${glitter.getUrlParameter('page')}`.startsWith(`collections`) ||
            `${glitter.getUrlParameter('page')}`.startsWith(`all-product`)
          ) {
            return await new Promise((resolve, reject) => {
              glitter.getModule(
                new URL('./public-components/product/product-list.js', gvc.glitter.root_path).href,
                res => {
            ...
            return await new Promise((resolve, reject) => {
              glitter.getModule(
                new URL('./public-components/product/product-detail.js', gvc.glitter.root_path).href,
                res => {
            ...
            //條款頁面
            let lan_d = (await ApiUser.getPublicConfig(`terms-related-${page}-${Language.getLanguage()}`, 'manager'))
              .response.value.text;
            if (!lan_d) {
              lan_d = (
                await ApiUser.getPublicConfig(
                  `terms-related-${page}-${(window as any).store_info.language_setting.def}`,
                  'manager'
                )
              ).response.value.text;
            }
