        ...
        return gvc.bindView({
            ...
            view: () => {
                if (loadings.view) {
                    ...
                    return html `
                                ...
                                return {
                                    ...
                                    view: () => {
                                        if (loading) {
                                            ...
                                            return html `<div style="text-align: center; vertical-align: middle;"><img src="${img}" /></div>`;
                                        }
                                    },
                                    divCreate: {},
                                    onCreate: () => {
                                        if (loading) {
                                            const si = setInterval(() => {
                                                ...
                                                if (qr) {
                                                    qr.toDataURL(`${vm.data.userID}`, { width: 400, margin: 2 }, (err, url) => {
                                                        if (err) {
                                                            console.error(err);
                                                            return;
                                                        }
                        ...
                        return html `<div class="um-title mb-1">目前累積消費金額</div>
                            ...
                            return `
                                                            background: ${(_a = glitter.share.globalValue['solid-button-bg']) !== null && _a !== void 0 ? _a : '#292218'};
                                ...
                                ${[
                        {
                            ...
                            title: '姓名',
                            type: 'text',
                            value: vm.data.userData.name,
                        },
                        {
                            key: 'account',
                            title: '信箱 / 帳號',
                            type: 'text',
                            value: vm.data.userData.email,
                        },
                        {
                            key: 'phone',
                            title: '手機',
                            type: 'text',
                            value: vm.data.userData.phone,
                        },
                        {
                            key: 'birth',
                            title: '生日',
                            type: 'date',
                            value: vm.data.userData.birth,
                        },
                    ]
                        .map((item) => {
                        var _a;
                        return html `
                                            <div class="form-group mb-3 col-12 col-md-6">
                                                <label class="um-content mb-1" for="${item.key}">${item.title}</label>
                                                <input class="bgw-input" type="${item.type}" id="${item.key}" placeholder="請輸入${item.title}" value="${(_a = item.value) !== null && _a !== void 0 ? _a : ''}" />
                                            </div>
                                        `;
                    })
                        .join('')}
            ...
            onCreate: () => {
                if (loadings.view) {
                    ...
                    ApiUser.getPublicConfig('custom_form_register', 'manager').then((data) => {
                        console.log('custom_form_register');
                        console.log(data);
                    });
