                                ...
                                <div class="container ${gClass('container')}">
                                                                                        ...
                                                                                        const spec = (() => {
                                                                                            if (item.spec) {
                                                                                                return item.spec.map((dd: string, index: number) => {
                                                                                                    try {
                                                                                                        return item.specs[index].option.find((d1: any) => {
                                                                                                            return d1.title === dd
                                    ...
                                    <section
                                        ...
                                        ${gvc.bindView(() => {
                                            ...
                                            vm_info.list=(widget.share.custom_form_checkout as any) ?? vm_info.list
    ...
    public static checkFormData(cartData: any, widget: any) {
        ...
        return new Promise((resolve) => {
            ...
            function checkString(text: any, errorMessage: any, type = '') {
                if ((pass && !text) || text === '') {
                    pass = false;
                    widget.event('error', {title: `${Language.text('please_enter')}「${errorMessage}」`});
                }
                if (pass && type === 'email' && !checkEmailPattern(text)) {
                    pass = false;
                    widget.event('error', {title: `「${errorMessage}」${Language.text('format_error')}`});
                }
                ...
                if (pass && type === 'name' && !checkReceiverPattern(text)) {
                    pass = false;
                    widget.event('error', {title: Language.text('name_length_restrictions')});
                }
            }
            ...
            if (subData['shipment'] === 'normal' && (!subData['address'] || subData['address'] === '')) {
                widget.event('error', {title: `${Language.text('please_enter')}「${Language.text('shipping_address')}」`});
            } else if (subData['shipment'] === 'normal' && !checkAddressPattern(subData['address'])) {
                widget.event('error', {title: Language.text('address_length_requirements')});
            } else if (['UNIMARTC2C', 'FAMIC2C', 'HILIFEC2C', 'OKMARTC2C'].includes(subData['shipment']) && !subData['CVSStoreName']) {
                widget.event('error', {title: Language.text('select_delivery_store')});
            } else if (
                (() => {
                ...
                widget.event('error', {title: `${Language.text('please_enter')}「${FormWidget.checkLeakData(form, subData)}」`});
            } else if (
                (() => {
                    return FormWidget.checkLeakData(cartData.custom_form_format, cartData.custom_form_data);
                })()
            ) {
                widget.event('error', {title: `${Language.text('please_enter')}「${FormWidget.checkLeakData(cartData.custom_form_format, cartData.custom_form_data)}」`});
            } else if (
                (() => {
                ...
                widget.event('error', {title: Language.text('please_select_gift')});
            } else {
