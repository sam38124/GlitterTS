...
export class FooterInitial {
  public static initial(cf: { browser: () => string; mobile: () => string, gvc: GVC }) {
    ...
    if(gvc.glitter.share.is_application){
        ...
        const res_=(()=>{
            if (((window as any).glitter.getUrlParameter('page').startsWith('products/') || ['account_userinfo','account_edit','order_list','voucher-list','rebate'
            ,'wishlist'].includes((window as any).glitter.getUrlParameter('page'))) && document.body.clientWidth < 800) {
                return true;
