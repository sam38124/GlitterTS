...
${EditorElem.arrayItem({
    ...
    refreshComponent: () => {
        ...
        },
        {
            title: '應用初始化 / Initial',
            index: 'pageConfig',
            html: `<div class="alert shadow" style="background-image: linear-gradient(120deg, #a1c4fd 0%, #c2e9fb 100%);white-space: normal;word-break: break-all;">${initialCode(
                gvc,
                viewModel,
                id
            )}</div>`,
...
${EditorElem.arrayItem({
    ...
    refreshComponent: () => {
        ...
        },
        {
            ...
            index: 'initialStyle',
            html: `<div class="alert shadow" style="background-image: linear-gradient(120deg, #a1c4fd 0%, #c2e9fb 100%);white-space: normal;word-break: break-all;">${initialStyle(
                gvc,
                viewModel,
                id
            )}</div>`,
 ...
 <ul class="nav nav-tabs border-bottom " id="myTab" role="tablist">
  ${(() => {
    ...
    <button class="nav-link ${index === 0 ? `active` : ``}"  data-bs-toggle="tab" data-bs-target="#${dd.index}" type="button" role="tab" aria-controls="${dd.index}" aria-selected="${index === 0}">${
                  dd.title
              }</button>
  </li>`;
          })
                ...
                view: () => {
                    return html`
                        <div class="d-flex border-bottom ">
                            ${tabIndex
                                .map((dd: { title: string; key: string }) => {
                                    return html` <div
...
<span class="mt-2">載入中...</span>
            ...
            return (
                ` <div class="alert alert-info m-2 p-3" style="white-space: normal;word-break: break-all;">透過資源管理工具，您能上傳多種檔案資源，在資源選擇器中快速選擇並且取用資源。</div>` +
                EditorElem.arrayItem({
                    gvc: gvc,
                    title: '',
                    array: () => {
