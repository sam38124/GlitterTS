...
export class FontsConfig {
    static fontsSettingView(gvc, globalValue, detail) {
        ...
        return gvc.bindView(() => {
            var _a;
            globalValue.font_theme = (_a = globalValue.font_theme) !== null && _a !== void 0 ? _a : [];
            const vm_c = {
                toggle: detail !== null && detail !== void 0 ? detail : false,
...
<div class="spinner-border"></div>
                    ...
                    const array = [html `
                        <div class="hoverF2 d-flex align-items-center p-3 ${detail ? `d-none` : ``}"
                             onclick="${gvc.event(() => {
                            vm_c.toggle = !vm_c.toggle;
                            gvc.notifyDataChange(vm_c.id);
                        })}">
<span class="fw-500"
      style="max-width: calc(100% - 50px);text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">字型樣式</span>
                            <div class="flex-fill"></div>
                            ${vm_c.toggle ? ` <i class="fa-solid fa-chevron-down"></i>` : ` <i class="fa-solid fa-chevron-right"></i>`}
                        </div>`];
                    if (vm_c.toggle) {
                        array.push(` <div class="row ${(globalValue.font_theme.length === 0) ? `d-none` : ``} px-0 pt-2 pb-0 m-0"
                                             style="margin: 18px 18px 0px;">
                                            ${globalValue.font_theme.map((dd, index) => {
                            return `<div class="col-12 mb-3" style="cursor: pointer;" >
                                               <div class="rounded border p-2 d-flex  w-100 flex-column" style="gap:10px;">
                                            ${BgWidget.title_16(index === 0 ? `預設字型` : dd.value || `字型樣式 ${index}`)}
                               <div class="d-flex align-items-center" style="gap:10px;">
                                ...
                                callback: (text) => {
                                    if (vm_c.fonts.find((dd, index) => { return dd.title === text; })) {
                                        dd.title = text;
                                        dd.value = text;
                                ...
                                placeHolder: '搜尋字體'
                                   ...
                                   ${(index !== 0) ? BgWidget.cancel(gvc.event(() => {
...
<div class="fs-6 bgf6 p-2" style="font-family: '${dd.value}' !important;">fonts</div>
    ...
    static reInitialFontTheme(globalValue, gvc) {
        const window = (gvc.glitter.document.querySelector('#editerCenter  iframe')).contentWindow;
        window.glitter.share.font_theme = globalValue.font_theme;
        window.glitter.share.font_theme.map((dd) => {
            if (!window.glitter.share.initial_fonts.find((d1) => {
                return d1 === dd.value;
            }) && dd.value) {
...
window.glitter.setModule(import.meta.url, FontsConfig);
