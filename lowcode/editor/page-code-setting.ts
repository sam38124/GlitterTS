...
export class PageCodeSetting {
    public static toggle(visible: boolean) {
        if (visible) {
            $('#codeSettingViewHover').removeClass('d-none')
            $('#PageCodeSettingView').removeClass('scroll-out')
            $('#PageCodeSettingView').addClass('scroll-in')
            PageCodeSetting.refresh()
        } else {
            PageCodeSetting.checkFinish(() => {
                $('#codeSettingViewHover').addClass('d-none')
                $('#PageCodeSettingView').addClass('scroll-out')
                $('#PageCodeSettingView').removeClass('scroll-in')
            })
    ...
    public static view(gvc: GVC) {
        ...
        return gvc.bindView(() => {
            return {
                bind: id,
                view: () => {
                    const originalData = [glitter.share.editorViewModel.data.config,
                        glitter.share.editorViewModel.globalStyle,
                        glitter.share.editorViewModel.globalScript].map((dd) => {
                        return JSON.stringify(dd)
                    })

                    PageCodeSetting.checkFinish=((callback) => {
                        const dialog = new ShareDialog(gvc.glitter)
                        if ([glitter.share.editorViewModel.data.config,
                            glitter.share.editorViewModel.globalStyle,
                            glitter.share.editorViewModel.globalScript].map((dd) => {
                            return JSON.stringify(dd)
                        }).join('') !== originalData.join('')) {
                            dialog.checkYesOrNot({
                                text: '偵測到變更，是否儲存?',
                                callback: (response) => {
                                    if (!response) {
                                        glitter.share.editorViewModel.data.config=JSON.parse(originalData[0])
                                        glitter.share.editorViewModel.globalStyle=JSON.parse(originalData[1])
                                        glitter.share.editorViewModel.globalScript=JSON.parse(originalData[2])
                                        callback()
                                    } else {
                            ...
                            return list.map((dd) => {
                                return `<div class="d-flex align-items-center justify-content-center hoverBtn  border"
                                 ...
                                 data-bs-title="${dd.label}" onclick="${gvc.event(() => {
                                    PageCodeSetting.checkFinish(() => {
                                        Storage.code_set_select = dd.key as any
                                        gvc.notifyDataChange(id)
                                    })
                                })}">
                               ${dd.icon}
