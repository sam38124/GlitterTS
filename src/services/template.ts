...
export class Template {
    ...
    public async createPage(config: { appName: string; tag: string; group: string; name: string; config: any; page_config: any; copy: any; page_type: string; copyApp: string; replace?: boolean }) {
        ...
        try {
            await db.execute(
                ...
                values (?, ?, ?, ?, ?, ?, ?, ?);
            `,
                [this.token!.userID, config.appName, config.tag, config.group, config.name, config.config ?? [], config.page_config ?? {}, config.page_type ?? 'page']
            );
            return true;
        } catch (e: any) {
