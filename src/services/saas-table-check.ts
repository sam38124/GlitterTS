...
export const SaasScheme = {
    createScheme: async () => {
        ...
        await trans.execute(`CREATE SCHEMA if not exists \`${saasConfig.SAAS_NAME}\` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ;`, {})
        await trans.execute(`
            CREATE TABLE if not exists \`${saasConfig.SAAS_NAME}\`.\`user\`
            (
                \`id\`
                int
                NOT
        ...
        `, {})
        await trans.execute(`CREATE TABLE if not exists \`${saasConfig.SAAS_NAME}\`.\`app_config\`
        (
            \`id\`
            int
            NOT
