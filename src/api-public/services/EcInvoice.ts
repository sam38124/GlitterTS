...
export class EcInvoice {
  //取得公司名稱
  public static getCompanyName(obj: { company_id: string; app_name: string }) {
    ...
    return new Promise<boolean>((resolve, reject) => {
      axios
        ...
        .then(async (response: any) => {
          ...
          console.log(`resp--->`, resp);
          await db.query(
            `insert into \`${obj.app_name}\`.t_invoice_memory
                          set ?`,
            [
              {
    ...
    return new Promise<boolean>((resolve, reject) => {
      axios
        ...
        .then(async (response: any) => {
          ...
          console.log(`resp--->`, resp);
          await db.query(
            `UPDATE \`${obj.app_name}\`.t_invoice_memory
                          set status = 2
      ...
      axios
        ...
        .then(async (response: any) => {
          ...
          if (resp.RtnCode != 1) {
            ...
            await db.query(
              `insert into \`${obj.app_name}\`.t_allowance_memory
                            set ?`,
              [
                {
                  status: '1',
