...
import { fakeProduct } from './fake-product.js';
...
export class FakeOrder {
  ...
  generateOrder(num = 500) {
    let data = [];
    const fakeData_ = fakeProduct;
    for (let a = 0; a < num; a++) {
      const user = getRandomElement(this.users);
      const orderID = `1${Tool.randomNumber(12)}`;
      let total = 0;
      const lineItems = (() => {
        let data = [];
      ...
      data.push([
        ...
        JSON.stringify({
          ...
          shipment_support: ['HILIFEC2C', 'FAMIC2C', 'normal', 'shop', 'sas3sesbs2s4s9sb'],
          shipment_selector: [
            {
              name: '一般宅配',
              value: 'normal',
