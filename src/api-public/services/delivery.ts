  ...
  async getOrderInfo(obj: { cart_token: string; shipment_date?: string }) {
    try {
      ...
      if (`${deliveryConfig.value.pay_now.toggle}` === 'true') {
        ...
        const log_number = await Promise.all(
          cart.map(async (dd: any) => {
            const that = this;

            async function updateShipmentNumber(numb: string) {
              ...
              await new Shopping(that.appName).putOrder({
        ...
        const shipment_config = await new User(this.appName).getConfigV2({
        ...
        if (shipment_config.bulk) {
          ...
          const request_={
            ...
            headers: { 'Content-Type': 'application/json' },
            data: {
              JsonOrder: await pay_now.encrypt(
                JSON.stringify({
        ...
        if (shipment_config.bulk) {
          const key = Tool.randomString(10);
          PayNowLogistics.printStack.push({
            code: key,
            html: link,
          });
          //5分鐘連結失效
          setTimeout(
            () => {
              PayNowLogistics.printStack = PayNowLogistics.printStack.filter(dd => dd.code !== key);
            },
            60 * 1000 * 5
          );
          return {
            result: true,
            link: `${process.env.DOMAIN}/api-public/v1/delivery/print-delivery?code=${key}&g-app=${this.appName}`,
          };
        } else {
          return {
            result: true,
