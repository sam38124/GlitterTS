...
export class LineMessage {
        ...
        try {
            ...
            await new Promise((resolve) => {
                for (const d of userList) {
                    this.sendLine({data:content , lineID:d.lineID , date:date}, (res)=>{
                        check--;
                        if (check === 0) {
    ...
    async sendLine(obj:{data: string, lineID: string , date?:string,order_id?:string } , callback: (data:any)=>void) {
        try {
            let postData = {
                "to": obj.lineID,
                "messages":
            ...
            throw exception.BadRequestError('BAD_REQUEST', 'send line Error:' + e, null);
        }
    }
    ...
    public async sendCustomerSns(tag: string, order_id: string, lineID: string) {
        const customerMail = await AutoSendEmail.getDefCompare(this.app, tag);
        if (customerMail.toggle) {
            await new Promise(async (resolve) => {
                resolve(await this.sendLine({data:customerMail.content.replace(/@\{\{訂單號碼\}\}/g, order_id) , lineID:lineID,order_id:order_id}, (res)=>{
