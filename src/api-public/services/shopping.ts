...
//
            ...
            if (query.sku) {
                querySql.push(`(id in ( select product_id from \`${this.app}\`.t_variants where content->>'$.sku'=${db.escape(query.sku)}))`);
            }
    ...
    async toCheckout(
        ...
        try {
            ...
            data.isExhibition = data.checkOutType === 'POS' && data.pos_store?.includes('exhibition_');
                                        ...
                                        try {
                                            if (data.checkOutType == "group_buy") {
                                                ...
                                                if (scheduledData) {
                                                    ...
                                                    if (productData) {
                                                        const variantData = productData.content.variants.find((dd: any) =>
                                                            dd.spec.join('-') === b.spec.join('-')
                                                        );
            ...
            // carData.lineItems.forEach((item:any)=>{
                ...
                if (data.checkOutType === 'group_buy') {
                    ...
                    async function getTempCart() {
                        try {
                            ...
                            console.log("GET t_temporary_cart error : ", err.response?.data || err.message)
                        }
                    }

                    async function updateCart(content: any) {
                        try {
                            await db.query(`
    ...
    async postVariantsAndPriceValue(content: any) {
        try {
            ...
            const insertPromises = content.variants.map(async (variant: any) => {
                ...
                const insertData = await db.query(`INSERT INTO \`${this.app}\`.t_variants
                                                   SET ?`, [{
                    content: JSON.stringify(variant),
                    product_id: content.id
                }]);

                const originalVariant = originVariants.find((item: any) => JSON.parse(item.spec).join(',') === variant.spec.join(','));
                            ...
                            if (content.shopee_id) {
                                ...
                                if (!shopee_dd) {
                                    shopee_dd = {
                                        ...
                                        data: await new Shopee(this.app, this.token).getProductDetail(content.shopee_id, {skip_image_load: true}),
                                    };
