...
export class Rebate {
  ...
  // 確認購物金功能是否啟用
  async mainStatus(): Promise<boolean> {
    try {
      const config = await App.checkBrandAndMemberType(this.app);
      if (config.plan === 'light-year') return false; // 如果是輕量方案，直接回傳 false

      const rs = await new User(this.app).getConfig({ key: 'rebate_setting', user_id: 'manager' });
      return Boolean(rs[0]?.value?.main);
    } catch (error) {
  ...
  async insertRebate(user_id: number | string, amount: number, note: string, proof?: RebateProof) {
    ...
    const deadTime =
      proof?.deadTime && Rebate.isValidDateTimeString(proof?.deadTime)
        ? moment(proof?.deadTime).format('YYYY-MM-DD HH:mm:ss')
        : '2999-12-31 00:00:00';
    const insertSQL = `
