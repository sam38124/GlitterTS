...
var UtDatabase = /** @class */ (function () {
    function UtDatabase(app, table) {
        this.app = app;
        this.table = table;
    }
    UtDatabase.prototype.querySql = function (querySql, query, select) {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        sql = "SELECT ".concat(select || '*', "\n                   FROM `").concat(this.app, "`.`").concat(this.table, "`\n                   where ").concat(querySql.map(function (dd) {
                            return "(".concat(dd, ")");
                        }).join(' and '), "\n                   ").concat(query.order_string ? query.order_string : "order by id desc");
                        if (!query.id) return [3 /*break*/, 2];
                        return [4 /*yield*/, database_js_1.default.query("SELECT  ".concat(select || '*', "\n                                          FROM (").concat(sql, ") as subqyery limit ").concat(query.page * query.limit, ", ").concat(query.limit), [])];
                    case 1:
                        data = (_b.sent())[0];
                        return [2 /*return*/, {
                                data: data,
