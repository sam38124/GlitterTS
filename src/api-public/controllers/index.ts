...
import app_market = require('./app-market');
...
router.use(config.getRoute(config.public_route.app_market, 'public'), app_market);
...
router.use(config.getRoute(config.public_route.progress, 'public'), require('./progress'));
...
const whiteList: {}[] = [
  ...
  { url: config.getRoute(config.public_route.ec + '/repay_redirect', 'public'), method: 'GET' },
