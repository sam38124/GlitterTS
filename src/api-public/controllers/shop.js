...
router.post('/checkout/repay', async (req, resp) => {
    try {
        return response_1.default.succ(resp, await new shopping_1.Shopping(req.get('g-app'), req.body.token).repayOrder(req.body.order_id, req.body.return_url));
    }
    catch (err) {
        return response_1.default.fail(resp, err);
    }
...
router.post('/notify', upload.single('file'), async (req, resp) => {
    try {
        ...
        if (type === 'paynow') {
            ...
            const data = await payNow.confirmAndCaptureOrder(check_id);
            if (data.type == 'success' && data.result.status === 'success') {
                const old_order_id = await redis_js_1.default.getValue(req.query.orderID);
                const idToQuery = old_order_id ? old_order_id : req.query.orderID;
                await new shopping_1.Shopping(req.query.appName).releaseCheckout(1, idToQuery);
            }
        }
        if (type === 'jkopay') {
...
router.put('/collection', async (req, resp) => {
    try {
        if (await ut_permission_1.UtPermission.isManager(req)) {
            ...
            return response_1.default.succ(resp, await new shopping_1.Shopping(req.get('g-app'), req.body.token).putCollectionV2(replace, original));
        }
        else {
            throw exception_1.default.BadRequestError('BAD_REQUEST', 'No permission.', null);
        }
...
router.put('/collection/sort', async (req, resp) => {
    try {
        if (await ut_permission_1.UtPermission.isManager(req)) {
            return response_1.default.succ(resp, await new shopping_1.Shopping(req.get('g-app'), req.body.token).sortCollectionV2(req.body.list));
        }
        else {
            throw exception_1.default.BadRequestError('BAD_REQUEST', 'No permission.', null);
        }
...
router.put('/product/variants', async (req, resp) => {
    try {
        if (await ut_permission_1.UtPermission.isManager(req)) {
            return response_1.default.succ(resp, await new shopping_1.Shopping(req.get('g-app'), req.body.token).putVariants(req.body.token, req.body));
        }
        else {
            throw exception_1.default.BadRequestError('BAD_REQUEST', 'No permission.', null);
        }
