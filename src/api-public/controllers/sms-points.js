...
const sms_pointes_js_1 = require("../services/sms-pointes.js");
...
router.get('/', async (req, resp) => {
    try {
        ...
        const data = await new ut_database_js_1.UtDatabase(req.get('g-app'), `t_sms_points`).querySql(query, req.query);
...
router.post('/', async (req, resp) => {
    try {
        const app = req.get('g-app');
        return response_1.default.succ(resp, await new sms_pointes_js_1.SmsPoints(app, req.body.token).store({
...
router.delete('/', async (req, resp) => {
    try {
        if (await ut_permission_js_1.UtPermission.isManager(req)) {
            await new sms_pointes_js_1.SmsPoints(req.get('g-app'), req.body.token).delete({
                id: req.body.id,
            });
...
router.put('/withdraw', async (req, resp) => {
    try {
        const app = req.get('g-app');
        if (await ut_permission_js_1.UtPermission.isManager(req)) {
            await new sms_pointes_js_1.SmsPoints(app, req.body.token).putWithdraw({
                id: req.body.id,
...
router.delete('/withdraw', async (req, resp) => {
    try {
        if (await ut_permission_js_1.UtPermission.isManager(req)) {
            await new sms_pointes_js_1.SmsPoints(req.get('g-app'), req.body.token).deleteWithDraw({
                id: req.body.id,
            });
            return response_1.default.succ(resp, {
                result: true,
...
router.get('/sum', async (req, resp) => {
    try {
        const app = req.get('g-app');
        return response_1.default.succ(resp, {
            sum: (await database_js_1.default.query(`SELECT sum(money) FROM \`${app}\`.t_sms_points
    ...
    try {
        if (await ut_permission_js_1.UtPermission.isManager(req)) {
            ...
            for (const b of req.body.userID) {
                await database_js_1.default.execute(`insert into \`${app}\`.t_sms_points (orderID, userID, money, status, note)
...
router.post('/notify', upload.single('file'), async (req, resp) => {
    try {
        ...
        if (decodeData['Status'] === 'SUCCESS') {
            await database_js_1.default.execute(`update \`${appName}\`.t_sms_points set status=? where orderID = ?
                `, [1, decodeData['Result']['MerchantOrderNo']]);
            const data = (await database_js_1.default.query(`select * from \`${appName}\`.t_sms_points where orderID=?`, [decodeData['Result']['MerchantOrderNo']]))[0];
...
//# sourceMappingURL=sms-points.js.map