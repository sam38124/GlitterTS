    ...
    if (await UtPermission.isManager(req)) {
      return response.succ(
        ...
        await new Recommend(req.get('g-app') as string, req.body.token).getLinkList({
          ...
          id_list: req.query.id_list ? `${req.query.id_list}` : '',
        })
      );
    } else {
...
router.put('/list/:id', async (req: express.Request, resp: express.Response) => {
  try {
    if (await UtPermission.isManager(req)) {
      return response.succ(
        resp,
        await new Recommend(req.get('g-app') as string, req.body.token).putLink(req.params.id, req.body)
      );
    } else {
      return response.fail(resp, exception.BadRequestError('BAD_REQUEST', 'No permission.', null));
    }
...
router.post('/list/toggle/:id', async (req: express.Request, resp: express.Response) => {
  try {
    if (await UtPermission.isManager(req)) {
      return response.succ(
        resp,
        await new Recommend(req.get('g-app') as string, req.body.token).toggleLink(req.params.id)
      );
    } else {
      return response.fail(resp, exception.BadRequestError('BAD_REQUEST', 'No permission.', null));
    }
...
router.put('/user/:id', async (req: express.Request, resp: express.Response) => {
  try {
    if (await UtPermission.isManager(req)) {
      return response.succ(
        resp,
        await new Recommend(req.get('g-app') as string, req.body.token).putUser(req.params.id, req.body)
      );
    } else {
      return response.fail(resp, exception.BadRequestError('BAD_REQUEST', 'No permission.', null));
    }
