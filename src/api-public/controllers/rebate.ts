...
router.post('/', async (req: express.Request, resp: express.Response) => {
  try {
    if (await UtPermission.isManager(req)) {
      ...
      if (amount !== 0) {
        const r = await rebateClass.insertRebate(
          req.body.user_id,
          amount,
          note && note.length > 0 ? note : '手動設定',
          req.body.proof
        );
        if (r?.result) {
...
router.post('/batch', async (req: express.Request, resp: express.Response) => {
  try {
    if (await UtPermission.isManager(req)) {
      ...
      const deadline =
        req.body.rebateEndDay !== '0'
          ? moment().add(req.body.rebateEndDay, 'd').format('YYYY-MM-DD HH:mm:ss')
          : undefined;
      const rebateClass = new Rebate(app);
