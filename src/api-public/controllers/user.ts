...
router.post('/manager/register', async (req: express.Request, resp: express.Response) => {
  try {
    ...
    const chunkSize = 20;
    const chunkedUserData = Array.from({ length: Math.ceil(userArray.length / chunkSize) }, (_, i) =>
      userArray.slice(i * chunkSize, (i + 1) * chunkSize)
    );
    let tempTags: string[] = [];
    ...
    for (const batch of chunkedUserData) {
      ...
      // 已存在則中止建立，並回傳
      if (errorResult) {
        throw exception.BadRequestError('BAD_REQUEST', errorResult.msg ?? 'User already exists', {
...
router.delete('/permission', async (req: express.Request, resp: express.Response) => {
  try {
    ...
    return response.succ(
      resp,
      await new SharePermission(req.get('g-app') as string, req.body.token).deletePermission(req.body.email)
    );
  } catch (err) {
    return response.fail(resp, err);
  }
...
router.put('/permission/status', async (req: express.Request, resp: express.Response) => {
  try {
    ...
    return response.succ(
      resp,
      await new SharePermission(req.get('g-app') as string, req.body.token).toggleStatus(req.body.email)
    );
  } catch (err) {
    return response.fail(resp, err);
  }
...
router.put('/permission/invite', async (req: express.Request, resp: express.Response) => {
  try {
    ...
    return response.succ(
      resp,
      await new SharePermission(req.get('g-app') as string, req.body.token).triggerInvited(req.body.email)
    );
  } catch (err) {
    return response.fail(resp, err);
  }
