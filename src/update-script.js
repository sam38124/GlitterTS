...
class UpdateScript {
    ...
    static async footer_migrate(app_name) {
        ...
        const header = (await database_1.default.query(`SELECT *
                                        FROM glitter.page_config
                                        where appName = ?
        ...
        if ((await database_1.default.query(`select count(1) from glitter.page_config where appName = ? and tag=?`, [app_name, 'index-mobile']))[0]['count(1)'] === 0) {
            const config = [
                {
                    "id": "ses0sbs0s8s9s0s3",
                    "js": "./official_view_component/official.js",
            ...
            b.name = 'APP首頁';
            await database_1.default.query(`insert into glitter.page_config set ?`, [b]);
        }
        console.log(`finish-${app_name}`);
    }
