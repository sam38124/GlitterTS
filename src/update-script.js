...
class UpdateScript {
    ...
    static async migrateSinglePage(appList) {
        ...
        for (const b of appList) {
            index2 = index2 + 1;
            await database_1.default.query(`delete from glitter.page_config where appName=? and tag=?`, [b, 'spa']);
            await database_1.default.query(`delete from glitter.page_config where appName=? and tag=?`, [b, 'spa' + index2]);
            await database_1.default.query(`delete from glitter.page_config where appName=? and tag=?`, ['shop_template_black_style', 'spa' + index2]);
            const image = (await database_1.default.query(`SELECT * FROM glitter.app_config where appName=?`, [b]))[0].template_config.image[0];
            const index = (await database_1.default.query(`select * from glitter.page_config where appName=? and tag=?`, [b, 'index']))[0];
            ...
            index.id = undefined;
            await database_1.default.query(`delete from shop_template_black_style.t_manager_post where id>0 and content->>'$.tag'=?`, [
                index.tag
            ]);
            await database_1.default.query(`insert into shop_template_black_style.t_manager_post set ?`, [
                {
                    userID: index.userID,
            ...
            const data = await database_1.default.query(`insert into glitter.page_config set ?`, [index]);
            await database_1.default.query(`replace into glitter.t_template_tag  (\`type\`,tag,bind) values (?,?,?)`, [
