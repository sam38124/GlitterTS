...
async function createAppRoute() {
  const apps = await database_2.default.execute(
    `SELECT appName
...
<<<<<<< HEAD
  ...
  return await glitter_util_js_1.GlitterUtil.set_frontend_v2(
    exports.app,
    ['/' + encodeURI(dd.appName) + '/*', '/' + encodeURI(dd.appName)].map(rout => {
      return {
        ...
        seoManager: async (req, resp) => {
          return await seo_config_js_1.SeoConfig.seoDetail(dd.appName, req, resp);
        },
        sitemap: async (req, resp) => {
          ...
          const domain = (
            await database_2.default.query(
              `select \`domain\`
=======
    const html = String.raw;
    live_source_1.Live_source.liveAPP.push(dd.appName);
                ...
                try {
                   ...
                   where LOWER(domain) = ?`, [og_url]))[0];
                    ...
                    if (data && data.page_config) {
                        ...
                        if (FBCode) {
                            if (!(req.headers['user-agent'].includes('iosGlitter') &&
                                !req.headers['user-agent'].includes('allow_track'))) {
...
>>>>>>> 02092ffb (feat: user list batch update event & progress api)
          ...
          const language_setting = (
            await new user_js_1.User(appName).getConfigV2({
              key: 'store-information',
              user_id: 'manager',
            })
          ).language_setting;
          const product = (
            await new shopping_js_1.Shopping(appName).getProduct({
              page: 0,
              limit: 100000,
              collection: '',
          ...
          const xml = await (0, sitemap_1.streamToPromise)(
              ...
              [
                ...
                ...(() => {
                  ...
                  (0, seo_config_js_1.extractCols)(cols).map(item => {
                    array = array.concat(
                      language_setting.support.map(d1 => {
                        const seo =
                          (item.language_data &&
        ...
        tw_shop: async (req, resp) => {
          ...
          const products = await database_2.default.query(
            `SELECT *
